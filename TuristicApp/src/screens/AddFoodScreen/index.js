...
import { createFood } from '../../services/servicesFood'
...
const AddFoodScreen = ({navigation}) => {
  ...
  const handleFoodSave = async() => {
    ...
    const responseUploadImages = await uploadImages(imagesSelected, "foods")
  ...
  const validForm = () => {
    ...
    if (isEmpty(formData.name)){
      setErrorName("Entering Foods name")
      isValid = false
    }
    ...
    if (isEmpty(formData.description)){
      setErrorDescription("Entering Foods description")
      isValid = false
    }
    if (isEmpty(formData.address)){
      setErrorAddress("Entering Foods address")
      isValid = false
    }
